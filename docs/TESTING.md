...
| `search-*` | `search --view files --json` returns the expected file for a markdown term, a root-level file, a nested file, a docx paragraph and a pptx speaker note; the payload has `query/results/timing_ms/view` and each result has `path, score, content_date, age_days, freshness_tier, date_source, project_path` (0.2.1 adds `role, date_basis, verify, noise, superseded_by, raw_similarity, why`; the smoke assertions are additive-compatible) |
| `recall-dry-run` | `recall --format text --query ...` against the smoke store prints a `<retrivio_leads>` block whose first lead is the expected note and writes a log line (the smoke config sets `recall_min_abs_score = 0.25`: hash vectors are not semantic and the note measures a cosine of about 0.29, under the Titan-calibrated default of 0.40) |
...
| `l2-search-json` | `retrivio search --view files --limit 3 --json "<content prompt>"` | at least one result; payload keys and per-result freshness fields as in L1, plus the slice 4 fields `role`, `date_basis`, `verify`, `noise`, `superseded_by`, `raw_similarity`, `why` |
| `l2-dossier-json` | `retrivio dossier --json --limit 5 "<dossier topic>"` | `schema` is `topic-dossier-v1`, `count >= 1`, every project has `entry.path`, `entry.role`, `evidence_count`, `reason`; `instruction` present. The topic defaults to "what do we know about" plus the content prompt's subject (`--dossier-topic` overrides) |
| `l2-recall-dossier-gate` | `retrivio recall --format text --query "<dossier topic>" ... --verbose` | the log line ends in `dossier:would-fire` or `dossier:no` (the gate ran); PASS on `would-fire`, WARN on `no` (the topic is not spread over three projects on this index: pick another `--dossier-topic`), FAIL when the token is missing (`recall_dossier` is `off`, or the gate regressed) |
| `l2-recall-instruction` | same with `--query "<instruction prompt>"` (default: "read the handoff, think about it deeply, brainstorm/ultrathink, and tell me what we should do next") | empty stdout, log `skipped:instruction`, no `dossier:` token |

The content prompt defaults to a question about Retrivio's own recall hook, which yields leads on any machine whose index covers the Retrivio repo. On another machine pass `--content-prompt "<question about something the index covers>"` (or `HARNESS_CONTENT_PROMPT`); keep it free of names you would not put in a public report. The dossier topic defaults to "what do we know about retrivio recall hooks"; pass `--dossier-topic "what do we know about <something spread over several of your projects>"` (or `HARNESS_DOSSIER_TOPIC`) to make the gate check PASS rather than WARN.
...
| `mcp-topic-dossier` | "nr: Call the Retrivio MCP tool topic_dossier with topic \"<dossier topic>\" and limit 5. Then reply with only the project_path of the first project ... else MCP-UNAVAILABLE." (Claude: `--allowedTools mcp__retrivio__topic_dossier`) | the reply's first line is an absolute path to a directory that is among the `project_path` values of `retrivio dossier --json` for the same topic (the `l2-dossier-json` run); Codex stderr must show `mcp: retrivio/topic_dossier (completed)` |
| `hook-dossier-gate` | "Reply exactly NONE. Do not use any tools. <dossier topic>" | the session's log line carries a `dossier:` token (`would-fire` PASS, `no` WARN as in L2); the reply is not inspected. With `recall_dossier = auto` the block would hold the compact dossier; the check does not require that |
| `hook-instruction` | "<instruction prompt>" (the default is the same prompt L2 uses; it carries no name, path or number, so the model just answers) | no block in the reply; log line `skipped:instruction`. The CLI may return non-zero when the model wanders on a bare instruction; only the log line and the block's absence are judged |
...
- `hook-dossier-gate` / `l2-recall-dossier-gate` with no `dossier:` token: `retrivio config show` must say `recall_dossier = shadow` or `auto`; in lexical fallback mode the gate has no cosines and reports `dossier:no`. A steady `no` on a topic you know is spread over projects: run `retrivio dossier --json "<topic>"` and read `best_cosine` per project; the gate needs three projects with the third within 0.10 of the first (see the README "Topic dossier").
- `hook-instruction` with a `semantic` log line: the instruction gate regressed (`recall.rs`, `is_instruction_prompt`); confirm with the L2 dry run. A prompt that names something (a path, a number, a capitalised word) is not an instruction by design.
- `mcp-topic-dossier`: the tool needs `--allowedTools mcp__retrivio__topic_dossier` in Claude print mode; a path that is a real directory but not in the CLI dossier's project list means the model answered from elsewhere.
...
Expected within one interval: a `watch: changes in <that project>` line naming only that project, then one tick `event updated=1 removed=0 ... files=<n>/<n-1>/1 ... chunks_embedded=<small> chunks_reused=0 chunks_deleted=0`, where `files=<selected>/<unchanged>/<rechunked>` shows exactly one file re-chunked (the event scan takes the manifest fast path; `files=81/0/81` means every file was re-read, which is the slice 4 regression), the embedded count is the edited file's chunk count and no other project appears. Over the next 30 minutes (six sweeps at `--interval 300`) the sweep ticks must show `updated=0` for an idle tree; the LanceDB version count (`ls ~/.retrivio/lance/*.lance/_versions | wc -l`) grows by one per write and drops back to a handful at the next sweep with a `sweep lancedb pruned: versions <before> -> <after>` line once the versions are older than `lance_version_grace_secs` (default 900 s); the data fragment count (`ls ~/.retrivio/lance/*.lance/data | wc -l`) grows by one or more per write and drops back with a `sweep lancedb compacted: versions ..., fragments <before> -> <after>` line once it passes `lance_compact_versions` (default 200, versions or fragments). The on-disk size (`du -sh ~/.retrivio/lance`) must return to about the live size within one sweep of a compaction; a size that stays near double the live size is the dead-fragment condition slice 4 fixed. Record the before/after counts and the tick lines. Finish with `retrivio service uninstall` if the machine must stay quiet, otherwise leave it running and note that in the record.

Failure signs: a tick listing several projects for one touched file (event filtering regressed), `files=n/0/n` on an event tick (fast path regressed), `updated>0` on idle sweeps (change gate regressed), a version or fragment count that only grows (pruning or compaction regressed), or `watch: index busy` repeating (another writer holds `index.lock`; stop it before judging the watcher).
